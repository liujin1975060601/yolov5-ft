...
from models.yolo import Detect, ValueInfer, SubclassInfer, Detect2
...
def run(
    ...
    if rknpu is not None:
        if opt.rknpu.upper() in ['RK1808', 'RV1109', 'RV1126','RK3399PRO']:
            os.environ['RKNN_model_hack'] = 'npu_1'
        elif opt.rknpu.upper() in ['RK3566', 'RK3568', 'RK3588','RK3588S','RV1106','RV1103']:
            os.environ['RKNN_model_hack'] = 'npu_2'
        else:
            assert False,"{} not recognized".format(rknpu)
    t = time.time()
    ...
    for k, m in model.named_modules():
        if isinstance(m, Detect):
            ...
            RK_anchors = m.stride.reshape(3,1).repeat(1,3).reshape(-1,1) * m.anchors.reshape(9,2)
            with open('anchors.txt', 'w') as anf:
                anf.write('\n'.join([','.join([f'{__v:.2f}' for __v in _v]) for _v in RK_anchors.numpy()]))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp335/weights/best.pt', help='model.pt path(s)')    # ROOT / 'yolov5s.pt'
    ...
    parser.add_argument('--rknpu', type=str ,default='rk1808', help='RKNN npu platform')
    opt = parser.parse_args()
    print_args('Export', opt)
    return opt
...
def export_rknn(weights, imgsz, data, qnt_num:int = 16, best_fit=True, precompile=True):
    name = os.path.split(weights)[-1].split('.')[0]
    output_dir = os.path.abspath(os.path.dirname(weights))
    qnt_dir = data if os.path.isdir(data) else None
    if qnt_dir is not None:    
        run(weights=weights,
            imgsz=imgsz,
            include=('onnx',),
            opset=11)
        command = f'docker run --rm -v {output_dir}:/workspace ' \
                f'-v {qnt_dir}:/qnt rktk171-e:v3 RKTK-E -w /workspace/{name}.onnx -d /qnt -b 4 -e {qnt_num // 4} '\
                f'{"-bf " if best_fit else ""}{"" if precompile else "-p"}'
        print(command)
        import subprocess
        ex = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        out, err = ex.communicate()
        if ex.returncode != 0:
            print('Export RKNN Error\n', out, '\n', err)
    else:
        print('Do not support ".txt" right now.')
...
if __name__ == "__main__":
    opt = parse_opt()
    main(opt)
    # export_rknn('yolov5-master/yolov5m.pt', None, None)
