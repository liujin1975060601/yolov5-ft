...
class ChannelAttentionModule(nn.Module):
    def __init__(self, c1, reduction=16):
        ...
        # self.sharedMLP = nn.Sequential(
        #     nn.Conv2d(c1, mid_channel, 1, bias=False),
        #     nn.SiLU(),
        #     nn.Conv2d(mid_channel, c1, 1, bias=False),
        # )
        self.sigmoid = nn.Sigmoid()
        #self.act=SiLU()
    def forward(self, x):
...
class CSPCAM(nn.Module):
        # CSP Bottleneck with 3 convolutions with cam
...
class CSPSAM(nn.Module):
        # CSP Bottleneck with 3 convolutions with sam
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
class CSPC(nn.Module):
        # CSP Bottleneck with 3 convolutions with CBAM
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
    ...
    def __init__(self, weights='yolov5s.pt', device=None, dnn=True):
        ...
        if jit:  # TorchScript
            ...
            for key, num in zip(['SubclassInfer', 'ValueInfer'], [1, 'nv']):
                p = model.module.get_module_byname(key) if hasattr(model, 'module') else model.get_module_byname(key)
                if p:
                ...
                del p
            p = model.module.get_module_byname('FTInfer') if hasattr(model, 'module') else model.get_module_byname('FTInfer')
            ft_infer = p is not None
            ft_length = p.ft_length if ft_infer else 0
            del p
...
class AutoShape(nn.Module):
    ...
    def _apply(self, fn):
        ...
        m = self.model.get_module_byname('Detect')  # Detect()
